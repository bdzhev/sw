  ...
  <fieldset v-bind="$attrs" class="min-w-0 border-0 p-0">
    ...
    >
      <ToggleGroupItem
        ...
        :key="option.value"
        :value="option.value"
        class="flex min-h-10 cursor-pointer items-center gap-1 rounded-full border border-border px-3 text-sm text-secondary transition-colors hover:text-primary disabled:cursor-not-allowed disabled:opacity-50 data-[state=on]:border-accent-primary data-[state=on]:bg-accent-primary/15 data-[state=on]:text-primary md:min-h-8"
      >
