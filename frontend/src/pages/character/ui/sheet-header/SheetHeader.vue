...
const hitDice = computed(() => {
  return `${props.sheet.hitDiceRemaining}/${hitDiceTotal(props.sheet)} d${HIT_DIE_BY_CLASS[props.character.characterClass]}`;
});
</script>
  ...
  >
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        :class="
          props.sheet.inspiration
            ? 'bg-accent-primary text-primary-button'
            : 'border border-border text-secondary'
        "
        @click="emit('patch', { inspiration: !props.sheet.inspiration }, true)"
      >
        Inspiration
      </button>
