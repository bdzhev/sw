import type { characterClassEnum, characterRaceEnum } from '../../db';
...
export const calculateStats = (
  ...
  const points = ALL_STATS.reduce((acc, stat) => {
    return { ...acc, [stat]: 0 };
  }, {} as CharacterStats);
  ...
  const totalWeight = Object.values(weighted).reduce((a, b) => {
    return a + b;
  }, 0);
  ...
  let total = Object.values(allocated).reduce((a, b) => {
    return a + b;
  }, 0);

  const sortedStats = [...ALL_STATS].sort((a, b) => {
  ...
  while (total < MAX_TOTAL_POINTS) {
    for (const stat of sortedStats) {
      if (
        allocated[stat] < MAX_POINT_PER_STAT &&
        !RESTRICTED_VALUES.includes(allocated[stat] + 1)
      ) {
        allocated[stat]++;
        total++;
        break;
  ...
  return ALL_STATS.reduce((acc, stat) => {
    acc[stat] =
      (pointsToAbilityScores[allocated[stat]] ?? 8) + (racialBonus[stat] ?? 0);
